...
# where cancer is not available let make it false
def work_for_cancer(dataset):
    dataset['cancerPresent']=np.where((dataset['cancerPresent'])==True,True,False)
    return dataset
# unit function for Cancer
dataset=work_for_cancer(dataset)
...
def write_final_csv(df):
    df.to_csv("false.csv",index=False)
    print("File Wrriten")
...
assert pd.read_csv("false.csv").empty==False
