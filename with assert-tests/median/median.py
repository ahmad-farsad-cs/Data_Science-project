...
assert 'first_name' not in dataset
...
assert dataset['patient_id'].dtype=='int64'
...
assert dataset['glucose_mg/dl_t1'].mean( )!=np.nan
...
assert min(dataset['atrophy_present'] )==0



# Replace null with median
def null_with_median(df):
    df =df.fillna(df.median())
    df['cancerPresent' ] =np.where((df['cancerPresent'] )==True ,True ,False)
    return df
dataset =null_with_median(dataset)

assert dataset['cancerPresent'].nunique( )==2
...
assert dataset['glucose_mg/dl_t3'].dtype=='float64'
...
assert dataset['glucose_mg/dl_average'].isna().sum( )==0
...
def diabetes_indication(dataset):
    dataset['diabetes_indication' ] =np.where((dataset['glucose_mg/dl_average'] ) <140 ,'normal' ,
        (np.where(dataset['glucose_mg/dl_average' ] >200 ,'diabetes' ,'prediabetes')))
    return dataset
...
assert dataset.diabetes_indication.dtype=='O'
...
def write_final_csv(df):
    df.to_csv("median.csv" ,index=False)
    print("File Wrriten")
...
assert pd.read_csv("median.csv").empty==False
